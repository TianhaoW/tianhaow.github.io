...
def sort_by_prefix_number(files):
    ...
    for category in sorted(os.listdir(MATH_ROOT)):
        ...
        for topic in sorted(os.listdir(cat_path)):
            ...
            topic_entry = {
                ...
                "link": f"/math/{category}/{topic}/",
                "items": []
            }

            note_files = [f for f in os.listdir(topic_path) if f.endswith(".md")]
            for note_file in sort_by_prefix_number([Path(f) for f in note_files]):
...
def generate_topic_index(topic_path: Path, category: str, topic: str) -> str:
    ...
    if not files:
        return (
            ...
            f"permalink: /math/{category}/{topic}/\n"
            "---\n\n"
...
def generate_category_index(cat_path: Path, category: str) -> str:
    ...
    for topic_path in sorted(cat_path.iterdir()):
